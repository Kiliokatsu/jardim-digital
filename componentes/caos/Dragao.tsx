...
import { useAtributo } from "@/componentes/Chaves";

/* O dragão em chamas do modo caos (DEC-0010).

   Adaptado do código de referência em `referencias/` (a captura da DEC-004):
   três desenhos definidos uma vez em <defs> e reutilizados por 39 <use> em
   follow-the-leader — cada segmento persegue o anterior, e o ângulo entre
   eles (atan2) é o que faz a ondulação de serpente. O fogo não é partícula:
   é paleta de brasa nos gradientes, drop-shadow no CSS (.dragao) e uma
   tremulação de seno na escala de cada segmento.

   O que ficou de fora da adaptação, de propósito: o fundo pintado, o
   cursor: pointer e o touch-action: none da página-demo — aqui o dragão é
   overlay com pointer-events: none, sobrevoa sem roubar clique nem rolagem. */
...
const VELOCIDADE_TREMULACAO = 0.18;

/* Quando o mouse para, o alvo passa a orbitar o centro (curva de Lissajous:
   cos(3t) × sin(4t)) — é o "vaguear sozinho" do original, mantido intacto. */
const FASE_POR_QUADRO = 0.003;
const RAIO_DE_VAGUEIO = 60;

function useMovimentoReduzido(): boolean {
  const inscrever = useCallback((avisar: () => void) => {
  ...
  // no servidor, o palpite conservador: menos movimento até o cliente dizer
  return useSyncExternalStore(inscrever, ler, () => true);
}

/** O portão: fora do caos (ou com movimento reduzido) o dragão nem existe no DOM. */
export function Dragao() {
  const [persona] = useAtributo("data-persona", "persona", "normal");
  const reduzido = useMovimentoReduzido();
  if (persona !== "caos" || reduzido) return null;
  return <CenaDragao />;
}
...
function CenaDragao() {
  ...
  /* O laço de quadro corre POR FORA do React, escrevendo transform direto
     nos nós: passar 39 atualizações por quadro pelo estado seria pedir 60
     re-renderizações por segundo. O React monta a cena; o rAF a move.
     Pelo mesmo motivo, os pontos dos segmentos são mutados in-place — é
     estado de animação, não dado da aplicação. */
  useEffect(() => {
    const svg = raizRef.current;
    if (!svg) return;

    const usos: SVGUseElement[] = [];
    svg.querySelectorAll<SVGUseElement>("use[data-i]").forEach((u) => {
      usos[Number(u.dataset.i)] = u;
    });
    ...
    const segmentos = Array.from({ length: SEGMENTOS }, () => ({
      x: largura / 2,
      y: 0,
    }));
    const alvo = { x: largura / 2, y: altura / 2 };
    let fase = Math.random(); // cada visita começa num ponto diferente da órbita
    let raio = 0; // cresce até raioMaximo; zera a cada toque do mouse
    let tempo = 0;
    ...
    const passo = () => {
      ...
      // a cabeça persegue o alvo com amortecimento; o desvio de Lissajous
      // só aparece conforme `raio` cresce (mouse parado)
      const cabeca = segmentos[0];
      const desvioX = (Math.cos(3 * fase) * raio * largura) / altura;
      const desvioY = (Math.sin(4 * fase) * raio * altura) / largura;
      cabeca.x += (desvioX + alvo.x - cabeca.x) / 10;
      cabeca.y += (desvioY + alvo.y - cabeca.y) / 10;
      ...
      for (let i = 1; i < SEGMENTOS; i++) {
        ...
        usos[i].setAttribute(
          "transform",
          `translate(${(anterior.x + seg.x) / 2} ${(anterior.y + seg.y) / 2}) ` +
            `rotate(${(angulo * 180) / Math.PI}) scale(${escalaBase * chama})`,
        );
      }

      if (raio < raioMaximo) raio++;
      fase += FASE_POR_QUADRO;
      if (raio > RAIO_DE_VAGUEIO) {
        // sem mouse por perto, o alvo deriva de volta pro centro da tela
        alvo.x += (largura / 2 - alvo.x) * 0.05;
        alvo.y += (altura / 2 - alvo.y) * 0.05;
      }
    };
  ...
  /* Ordem de pintura do SVG é ordem do documento: a cauda entra primeiro e a
     cabeça por último, pra cabeça passar POR CIMA do corpo nas curvas. */
  const indices = Array.from({ length: SEGMENTOS - 1 }, (_, k) => SEGMENTOS - 1 - k);

  return (
    <svg ref={raizRef} className="dragao" aria-hidden="true">
      <defs>
        {/* Paleta de brasa (DEC-0010): amarelo incandescente perto do corpo,
            morrendo no vermelho do caos (#ff2d55) — mudar o acento do caos
            muda o fogo junto, e é assim que deve ser. */}
        <linearGradient
          id="dragao-grad-cabeca"
        ...
        {/* As asas viram labaredas: vivas junto do corpo, dissipando na ponta
            (o vermelho vai a opacidade quase zero, como chama no ar). */}
        <linearGradient
          id="dragao-grad-asa"
          gradientUnits="userSpaceOnUse"
        ...
        >
          ...
          <stop offset="1" stopColor="#ffb84d" stopOpacity="0.95" />
        </linearGradient>
        <g id="dragao-asa">
          <path
            fill="url(#dragao-grad-asa)"
        ...
        {/* A espinha em brasa: laranja no eixo, vermelho profundo na borda.
            São dois gradientes porque o desenho original é espelhado em Y. */}
        <linearGradient
          id="dragao-grad-espinha-a"
          gradientUnits="userSpaceOnUse"
          ...
          y2="0"
        >
          <stop offset="0" stopColor="#ffc25e" />
          <stop offset="1" stopColor="#b3122e" />
        </linearGradient>
        <linearGradient
          id="dragao-grad-espinha-b"
          gradientUnits="userSpaceOnUse"
          gradientTransform="matrix(0.0229492, 0, 0, 0.0152893, 0, -0.05)"
          spreadMethod="pad"
          x1="-819.2"
        ...
        <g id="dragao-espinha">
          <path
            fill="url(#dragao-grad-espinha-a)"
          ...
          />
          <path
            fill="url(#dragao-grad-espinha-b)"
      ...
      {indices.map((i) => (
        <use
          ...
          href={
            i === 1
              ? "#dragao-cabeca"
              : INDICES_DE_ASA.has(i)
                ? "#dragao-asa"
                : "#dragao-espinha"
          }
          /* fogo esmorece rumo à cauda: a ponta é fumaça, não chama */
          opacity={0.55 + 0.45 * (1 - i / (SEGMENTOS - 1))}
          /* nasce fora da tela; o primeiro quadro do rAF posiciona de verdade */
